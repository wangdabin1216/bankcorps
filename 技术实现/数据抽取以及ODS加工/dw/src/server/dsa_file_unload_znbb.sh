...
#导出数据
beeline -u ${HIVE_JDBC} -n ${HIVE_USER} -p ${HIVE_PASSWORD} -e "begin set_env('transaction.type','inceptor'); set_env('mapred.reduce.tasks','5');insert overwrite directory '$hdfs_path/$table_name' row format delimited fields terminated by '|' select * from omi.$table_name where updt_date='$currdt' distribute by rand() end"
if [ $? -eq 0 ];then
