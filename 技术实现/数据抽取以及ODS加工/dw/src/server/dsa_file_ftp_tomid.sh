#/bin/bash
################################################################################
#脚本名称：dsa_file_ftp_tomid.sh 
#脚本功能：对外供数的FTP传输脚本
#输入参数：供数系统 供数日期 eg: sh dsa_file_ftp_tomid.sh kn 20190228
#编写人    ：guxn
#编写日期：2020-03-10
#修改记录：
...
#获取FTP服务器连接信息
t_ip=`$mysql_conn_str "select trim(para_val) from etl_meta_args where parameter='dsa.ftp.mid.ip'"`
t_user=`$mysql_conn_str "select trim(para_val) from etl_meta_args where parameter='dsa.ftp.mid.user'"`
t_pwd=`$mysql_conn_str "select trim(para_val) from etl_meta_args where parameter='dsa.ftp.mid.pwd'"`
t_spath=`$mysql_conn_str "select trim(para_val) from etl_meta_args where parameter='dsa.ftp.mid.spath'"`
t_tpath=`$mysql_conn_str "select trim(para_val) from etl_meta_args where parameter='dsa.ftp.mid.tpath'"`
if [ "$t_ip"x != "x" ];then
