...
#获取源系统表数据文件
file_name_tmp=`cat $conf_path/table.conf | grep "\<${tablename}\>" | awk -F ',' '{print $1}' `
load_data_file=`echo ${file_name_tmp//'${currdt}'/$currdt}.dat`
echo `date +"%Y-%m-%d %H:%M:%S"`"[INFO]源系统文件名为：$load_data_file">> $log_path/etl_$tablename$currdt.log
cp $target_path/$load_data_file  $put_path/$tablename.txt
echo `date +"%Y-%m-%d %H:%M:%S"`"[EXECUTE]cp $target_path/$load_data_file  $put_path/$tablename.txt" >> $log_path/etl_$tablename$currdt.log
...
#codec=`sqlite3 $sqlite_path/etl.db "select encode_format from etl_load_table where lower(sys||'_'||tab_name)=lower('$tablename');"`
