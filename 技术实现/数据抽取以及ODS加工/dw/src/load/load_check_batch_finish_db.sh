#/bin/bash 
#脚本名称：load_check_batch_finish_db.sh
#脚本功能：检测核心系统批量任务表当日批量是否完成
#输入参数：日期 eg: sh load_check_batch_finish_db.sh sys 20190228
#编写人    ：guxn
#编写日期：20200311
#修改记录：
...
echo `date +"%Y-%m-%d %H:%M:%S"`"[INFO]日期为：$currdt"> $log_path/etl_load_check_batch_finish_db_${sys}.log
...
#判断读取参数是否成功
 if [ "$oracle_db"x = "x" -o "$oracle_name"x = "x" -o "$oracle_password"x = "x" -o "$oracle_ip"x = "x" -o "$oracle_port"x = "x" -o "$schema_name"x = "x" ];then
   echo `date +"%Y-%m-%d %H:%M:%S"`"[ERROR]读取参数connect_str错误">> $log_path/etl_load_check_batch_finish_db_${sys}.log
   exit 1
 fi
echo `date +"%Y-%m-%d %H:%M:%S"`"[INFO]数据库连接串为：$connect_url">> $log_path/etl_load_check_batch_finish_db_${sys}.log
...
if [ "${szStatus}" == "1" ];then
   echo `date +"%Y-%m-%d %H:%M:%S"`"[INFO]${sys}系统日终完成">> $log_path/etl_load_check_batch_finish_db_${sys}.log
    exit 0
else
    echo `date +"%Y-%m-%d %H:%M:%S"`"[INFO]${sys}系统日终未完成">> $log_path/etl_load_check_batch_finish_db_${sys}.log
    exit 200
fi
